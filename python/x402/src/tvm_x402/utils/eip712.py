...
from tvm_x402.types import KIND_MAP, PaymentPermit
...
def payment_id_to_bytes(payment_id: str) -> bytes:
    ...
    if not payment_id.startswith("0x"):
        raise ValueError(f"Invalid payment ID format: {payment_id}. Expected hex string with 0x prefix")
    
    payment_id_hex = payment_id[2:]
    if len(payment_id_hex) != 32:
        raise ValueError(f"Invalid payment ID length: {len(payment_id_hex)}. Expected 32 hex characters")
