"""
TronFacilitatorChainMixin - Shared chain-read and contract logic for TRON facilitator signers.

Provides:
- AsyncTron client management
- EIP-712 signature verification
- Contract write orchestration (delegates signing to subclass hook)
- Transaction receipt waiting
- Address normalization and logging helpers
"""
...
class TronFacilitatorChainMixin:
    """Mixin providing shared TRON facilitator chain-read and contract logic.

    Subclasses must:
    - Set ``self._address: str`` and ``self._network: str | None``
    - Initialise ``self._async_tron_clients: dict[str, Any] = {}``
    - Implement ``_sign_transaction``
    """
    ...
    async def write_contract(
        ...
        Builds the transaction locally, delegates signing to
        ``_sign_transaction``, then broadcasts.
        """
        ...
        try:
            ...
            # Delegate signing to subclass, get back a broadcastable transaction
            signed_txn = await self._sign_transaction(txn)
    ...
    async def _sign_transaction(self, txn: Any) -> Any:
        """Sign a built transaction and return a broadcastable transaction object.

        Args:
            txn: Built (unsigned) tronpy transaction.

        Returns:
            Signed transaction object that supports ``.broadcast()``.
        """
        ...
