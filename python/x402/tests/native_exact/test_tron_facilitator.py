...
from x402_tron.mechanisms.native_exact.tron import NativeExactTronFacilitatorMechanism
...
def mock_signer():
    ...
    signer.write_contract = AsyncMock(return_value="txhash_tron_native_exact")
    signer.wait_for_transaction_receipt = AsyncMock(
        return_value={"hash": "txhash_tron_native_exact", "status": "confirmed"}
    )
    return signer
...
class TestTokenWhitelist:
    ...
    async def test_allowed_token_passes(self, mock_signer, nile_requirements):
        mechanism = NativeExactTronFacilitatorMechanism(mock_signer, allowed_tokens={USDT_ADDRESS})
        payload = _make_payload(nile_requirements)
        result = await mechanism.verify(payload, nile_requirements)
        assert result.is_valid is True
...
class TestSettle:
    ...
    async def test_settle_success(self, mock_signer, nile_requirements):
        ...
        assert result.transaction == "txhash_tron_native_exact"
        assert result.network == "tron:nile"
